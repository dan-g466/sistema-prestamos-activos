<x-user-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-700">
            <div>
                <div class="inline-flex items-center gap-2 px-3 py-1 bg-amber-100 dark:bg-amber-900/40 rounded-full mb-3">
        ...
        @if($prestamos->count() > 0)
            ...
            @if($prestamos->where('estado', 'Por Confirmar')->count() > 0)
                <div class="mb-6 animate-in fade-in slide-in-from-left-4 duration-700">
                    <div class="bg-gradient-to-r from-indigo-600 to-blue-700 dark:from-indigo-900 dark:to-blue-900 rounded-3xl p-6 shadow-xl shadow-indigo-200 dark:shadow-none relative overflow-hidden flex items-center gap-6">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                                <th class="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Equipo</th>
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] hidden md:table-cell">Código</th>
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Rango de Fecha</th>
                                <th class="px-6 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Estado</th>
                                <th class="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] text-right">Seguimiento</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                            @foreach ($prestamos as $p)
                                <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors duration-300">
                                    <td class="px-8 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col gap-1">
                                            <div class="flex items-center gap-2">
                                                <span class="text-[8px] font-black text-slate-300 dark:text-slate-600 uppercase w-8">Inicio:</span>
                                                <span class="text-[10px] font-bold text-[#00324D] dark:text-slate-300">{{ $p->fecha_inicio ? $p->fecha_inicio->format('d/m/Y h:i A') : 'Por definir' }}</span>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <span class="text-[8px] font-black text-slate-300 dark:text-slate-600 uppercase w-8">Final:</span>
                                    ...
                                    <td class="px-6 py-4">
                                        @php
                                            $statusStyles = [
                                                'Pendiente' => 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-900/30',
                                                'Aceptado'  => 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-900/30',
                                                'Activo'    => 'bg-green-50 dark:bg-green-900/20 text-[#39A900] dark:text-green-400 border-green-100 dark:border-green-900/30',
                                                'Vencido'   => 'bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-900/30',
                                                'Por Confirmar' => 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-900/30',
                                            ];
                                            $currentStyle = $statusStyles[$p->estado] ?? 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-100 dark:border-slate-700';
                                        @endphp
                                    ...
                                    <td class="px-8 py-4 text-right">
                                        <a href="{{ route('user.prestamos.show', $p) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 dark:bg-slate-800 text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-[#00324D] dark:hover:bg-slate-700 hover:shadow-lg transition-all active:scale-95 group/btn border dark:border-slate-700">
                                            Proceso
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center py-24 text-center animate-in fade-in duration-1000">
                <div class="w-24 h-24 bg-slate-50 dark:bg-slate-900 rounded-[2.5rem] flex items-center justify-center mb-6 border border-slate-100 dark:border-slate-800 shadow-inner group">
