...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased overflow-hidden" 
      x-data="{ 
    ...
    <div class="flex h-screen relative z-10">
        ...
        <aside class="bg-[#39A900] dark:bg-slate-900 w-64 flex-shrink-0 flex flex-col fixed inset-y-0 z-50 transition-all duration-500 shadow-[0_0_50px_-15px_rgba(57,169,0,0.3)] dark:shadow-none dark:border-r border-slate-800/60"
            ...
            <div class="p-8 pb-4 relative overflow-hidden group">
                ...
                <div class="relative z-10 flex flex-col items-center gap-5 translate-y-0 group-hover:-translate-y-1 transition-transform duration-500">
                    <div class="group-hover:rotate-[5deg] transition-all duration-500 relative">
                        <img x-show="!darkMode" src="{{ asset('img/logo.png') }}" alt="Logo SENA" class="h-16 w-auto object-contain brightness-0 invert transition-all duration-500">
                        <img x-show="darkMode" src="{{ asset('img/logo sena.png') }}" alt="Logo SENA" class="h-16 w-auto object-contain transition-all duration-500" x-cloak>
                    </div>
            ...
            <nav class="flex-1 mt-8 px-4 space-y-2 overflow-y-auto custom-scrollbar">
                <a href="{{ route('user.dashboard') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold transition-all duration-300 group
                          {{ request()->routeIs('user.dashboard') ? 'bg-white dark:bg-emerald-600/20 text-[#39A900] dark:text-emerald-400 shadow-[0_10px_20px_-5px_rgba(255,255,255,0.4)] dark:shadow-none scale-[1.02]' : 'text-white/80 dark:text-slate-400 hover:bg-white/10 dark:hover:bg-slate-800 hover:text-white dark:hover:text-slate-200' }}">
                ...
                <a href="{{ route('user.catalogo') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold transition-all duration-300 group
                          {{ request()->routeIs('user.catalogo') ? 'bg-white dark:bg-emerald-600/20 text-[#39A900] dark:text-emerald-400 shadow-[0_10px_20px_-5px_rgba(255,255,255,0.4)] dark:shadow-none scale-[1.02]' : 'text-white/80 dark:text-slate-400 hover:bg-white/10 dark:hover:bg-slate-800 hover:text-white dark:hover:text-slate-200' }}">
                ...
                <a href="{{ route('user.prestamos.index') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold transition-all duration-300 group
                          {{ request()->routeIs('user.prestamos.index') ? 'bg-white dark:bg-amber-900/40 text-amber-600 dark:text-amber-500 shadow-[0_10px_20px_-5px_rgba(255,191,0,0.3)] dark:shadow-none scale-[1.02]' : 'text-white/80 dark:text-slate-400 hover:bg-white/10 dark:hover:bg-slate-800 hover:text-white dark:hover:text-slate-200' }}">
                ...
                <a href="{{ route('user.prestamos.activos') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold transition-all duration-300 group
                          {{ request()->routeIs('user.prestamos.activos') ? 'bg-white dark:bg-emerald-600/20 text-[#39A900] dark:text-emerald-400 shadow-[0_10px_20px_-5px_rgba(255,255,255,0.4)] dark:shadow-none scale-[1.02]' : 'text-white/80 dark:text-slate-400 hover:bg-white/10 dark:hover:bg-slate-800 hover:text-white dark:hover:text-slate-200' }}">
                ...
                <a href="{{ route('user.historial') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold transition-all duration-300 group
                          {{ request()->routeIs('user.historial') ? 'bg-white dark:bg-emerald-600/20 text-[#39A900] dark:text-emerald-400 shadow-[0_10px_20px_-5px_rgba(255,255,255,0.4)] dark:shadow-none scale-[1.02]' : 'text-white/80 dark:text-slate-400 hover:bg-white/10 dark:hover:bg-slate-800 hover:text-white dark:hover:text-slate-200' }}">
                ...
                <a href="{{ route('user.sanciones.index') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold transition-all duration-300 group
                          {{ request()->routeIs('user.sanciones.*') ? 'bg-white dark:bg-rose-900/20 text-rose-600 dark:text-rose-500 shadow-xl dark:shadow-none' : 'text-white/80 dark:text-slate-400 hover:bg-rose-500/20 dark:hover:bg-rose-900/30 hover:text-white dark:hover:text-rose-400' }}">
            ...
            <!-- Logout Estilizado -->
            <div class="mt-auto p-4 border-t border-white/10 dark:border-slate-800/60 bg-black/5 dark:bg-transparent backdrop-blur-sm">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-4 rounded-2xl text-[11px] font-black text-white dark:text-slate-400 hover:bg-white dark:hover:bg-slate-800 group transition-all hover:text-[#39A900] dark:hover:text-rose-500 active:scale-95 uppercase tracking-widest border border-white/20 dark:border-slate-800 shadow-lg dark:shadow-none">
        ...
        <div class="flex-1 flex flex-col overflow-hidden transition-all duration-500" :class="sidebarOpen && window.innerWidth >= 1024 ? 'lg:ml-64' : ''">
            ...
            <header class="h-20 glass-header sticky top-0 z-30 flex items-center justify-between px-8 border-b border-slate-200/60 dark:border-slate-800 shadow-sm transition-all duration-300">
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex items-center gap-4 pl-6 border-l border-slate-100 dark:border-slate-800 transition-colors">
                        <div class="text-right hidden md:block">
                            ...
                            <span class="text-[9px] font-black text-[#39A900] dark:text-emerald-400 uppercase tracking-widest">Activo • Aprendiz</span>
                        </div>
                        <div class="w-12 h-12 bg-gradient-to-br from-[#39A900] to-green-600 dark:from-emerald-600 dark:to-emerald-800 rounded-2xl flex items-center justify-center text-white text-xl font-black shadow-xl shadow-green-900/10 border-2 border-white dark:border-slate-800 ring-4 ring-slate-100 dark:ring-slate-900 transition-all hover:scale-110 cursor-pointer">
            ...
            <main class="flex-1 overflow-y-auto p-6 md:p-10 relative custom-scrollbar">
                @if(session('success'))
                    <div class="max-w-4xl mx-auto mb-8 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border-l-8 border-[#39A900] p-5 rounded-[1.8rem] shadow-xl shadow-green-900/5 dark:shadow-none animate-in slide-in-from-top-4 duration-500 overflow-hidden relative border dark:border-slate-800">
                        <div class="absolute right-0 top-0 w-24 h-full bg-gradient-to-l from-green-50 dark:from-[#39A900]/10 to-transparent"></div>
                        <div class="relative z-10 flex items-center gap-4">
                            ...
                            <p class="text-sm font-bold text-slate-800 dark:text-slate-200 tracking-tight">{{ session('success') }}</p>
                        </div>
                    </div>
                @endif
                {{ $slot }}
