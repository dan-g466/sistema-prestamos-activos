<x-admin-layout>
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-600 dark:text-slate-400 text-[10px] uppercase font-black tracking-widest mt-1">
                    Gestión de Préstamos y Devoluciones <span class="mx-2 text-slate-300 dark:text-slate-700">|</span> <span class="text-[#39A900]">SENA</span>
                </p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.prestamos.vencidos') }}" class="relative inline-flex items-center gap-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 hover:bg-rose-600 dark:hover:bg-rose-600 hover:text-white font-black py-2 px-4 rounded-xl transition-all duration-300 group text-[10px] uppercase tracking-widest border border-rose-100 dark:border-rose-900/30 shadow-sm shadow-rose-900/5 active:scale-95">
    ...
    @if(session('success') || session('error'))
        <div x-data="{ show: true }" 
            ...
            <div class="bg-white dark:bg-slate-900 border-l-4 {{ session('success') ? 'border-[#39A900]' : 'border-rose-500' }} shadow-[0_10px_30px_-5px_rgba(0,0,0,0.1)] rounded-2xl p-4 flex items-center justify-between group border border-slate-100 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    <div class="{{ session('success') ? 'bg-green-50 dark:bg-green-900/20' : 'bg-rose-50 dark:bg-rose-900/20' }} p-2 rounded-xl">
                        @if(session('success'))
                    ...
                    <div>
                        <h4 class="text-[10px] font-black uppercase tracking-widest text-[#00324D] dark:text-white leading-none mb-1">{{ session('success') ? '¡Éxito!' : 'Aviso' }}</h4>
                        <p class="text-[11px] font-bold text-slate-600 dark:text-slate-400 leading-tight">{{ session('success') ?? session('error') }}</p>
                    </div>
                </div>
                <button @click="show = false" class="text-slate-300 hover:text-slate-500 dark:text-slate-600 dark:hover:text-slate-400 transition-colors p-1">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-800">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50 uppercase tracking-widest">
                        <tr>
                            <th class="px-6 py-2 text-left text-[8px] font-black text-slate-600 dark:text-slate-400">Aprendiz / Solicitante</th>
                            <th class="px-6 py-2 text-left text-[8px] font-black text-slate-600 dark:text-slate-400">Elemento Técnico</th>
                            <th class="px-6 py-2 text-left text-[8px] font-black text-slate-600 dark:text-slate-400">Cronograma / Plazos</th>
                            <th class="px-6 py-2 text-left text-[8px] font-black text-slate-600 dark:text-slate-400">Estado</th>
                    ...
                    <tbody class="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
                        @forelse ($prestamos as $prestamo)
                            ...
                            @endphp
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group/row text-[11px] {{ $esVencido ? 'bg-rose-50/30 dark:bg-rose-900/10' : '' }}">
                                <td class="px-6 py-2">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[#00324D] dark:text-slate-300 font-black text-[10px] uppercase border border-slate-200 dark:border-slate-700">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[9px] text-slate-400 dark:text-slate-500 font-bold tracking-wider">ID: {{ $prestamo->user->documento }}</div>
                                        </div>
                                    </div>
                                </td>
                                
                                <td class="px-6 py-2">
                                    <div class="font-bold text-[#00324D] dark:text-slate-300">{{ $prestamo->elemento->nombre }}</div>
                                    <div class="flex items-center gap-1.5 mt-0.5">
                                        <span class="text-[8px] font-black px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded uppercase tracking-tighter border border-slate-200 dark:border-slate-700">PLACA</span>
                                        <span class="text-[9px] font-mono font-bold text-[#39A900]">{{ $prestamo->elemento->codigo_sena }}</span>
                                    </div>
                                </td>

                                <td class="px-6 py-2 font-medium">
                                    <div class="text-slate-400 dark:text-slate-500 text-[9px] uppercase tracking-tighter">Desde: {{ $prestamo->fecha_solicitud->format('d M, Y - H:i') }}</div>
                                    <div class="font-black mt-0.5 flex flex-col gap-1 {{ $esVencido ? 'text-rose-600 animate-pulse' : 'text-slate-600 dark:text-slate-400' }}">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            @else
                                                <svg class="w-3 h-3 text-slate-300 dark:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                            @endif
                                            Límite: {{ $prestamo->fecha_devolucion_esperada->format('d M, Y - H:i') }}
                                        </div>
                                        @if($prestamo->observaciones)
                                            <div class="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded px-1.5 py-0.5 text-[8px] font-medium text-slate-400 dark:text-slate-500 max-w-[180px] truncate" title="{{ $prestamo->observaciones }}">
                                ...
                                <td class="px-6 py-2">
                                    @if($esVencido)
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-rose-600 text-white rounded-full text-[8px] font-black uppercase tracking-widest shadow-sm">
                                            ...
                                            Vencido ({{ $tiempoAtrasado }})
                                        </span>
                                    @else
                                        @php
                                            $statusBadge = match($prestamo->estado) {
                                                'Pendiente' => ['bg' => 'bg-amber-100 dark:bg-amber-900/20', 'text' => 'text-amber-600 dark:text-amber-400', 'dot' => 'bg-amber-400'],
                                                'Aceptado'  => ['bg' => 'bg-green-100 dark:bg-green-900/20', 'text' => 'text-green-700 dark:text-green-400', 'dot' => 'bg-green-500'],
                                                'Activo'    => ['bg' => 'bg-[#39A900]/10 dark:bg-[#39A900]/20', 'text' => 'text-[#39A900]', 'dot' => 'bg-[#39A900]'],
                                                'Por Confirmar' => ['bg' => 'bg-indigo-100 dark:bg-indigo-900/20', 'text' => 'text-indigo-600 dark:text-indigo-400', 'dot' => 'bg-indigo-500'],
                                                'Devuelto'  => ['bg' => 'bg-slate-100 dark:bg-slate-800', 'text' => 'text-slate-400 dark:text-slate-500', 'dot' => 'bg-slate-300 dark:bg-slate-600'],
                                                'Rechazado' => ['bg' => 'bg-rose-100 dark:bg-rose-900/20', 'text' => 'text-rose-600 dark:text-rose-400', 'dot' => 'bg-rose-400'],
                                                default     => ['bg' => 'bg-slate-50 dark:bg-slate-800', 'text' => 'text-slate-400 dark:text-slate-500', 'dot' => 'bg-slate-200 dark:bg-slate-700']
                                            };
                                        @endphp
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 {{ $statusBadge['bg'] }} {{ $statusBadge['text'] }} rounded-full text-[8px] font-black uppercase tracking-widest border {{ str_replace('bg-', 'border-', $statusBadge['bg']) }} dark:border-current/10">
                                ...
                                <td class="px-8 py-2 text-right">
                                    <div class="flex justify-end items-center gap-1.5 transition-all">
                                        <a href="{{ route('admin.prestamos.show', $prestamo) }}"
                                           class="p-1 px-3 bg-slate-100 dark:bg-slate-800 text-[#39A900] dark:text-[#39A900] hover:bg-green-50 dark:hover:bg-slate-700 rounded-lg transition-all border border-slate-200 dark:border-slate-700 shadow-sm active:scale-95 flex items-center gap-1.5"
                                           title="Ver detalle">
                                        ...
                                        @if($prestamo->estado == 'Pendiente')
                                            ...
                                            <form action="{{ route('admin.prestamos.rechazar', $prestamo) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="p-1 px-3 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-lg hover:bg-rose-100 dark:hover:bg-rose-900/30 font-black text-[9px] uppercase tracking-widest transition-all active:scale-95 flex items-center gap-1.5" title="Rechazar">
                                        ...
                                        @elseif($prestamo->estado == 'Activo')
                                            <button onclick="abrirModalDevolucion('{{ $prestamo->id }}', '{{ $prestamo->elemento->nombre }}')" 
                                                    class="p-1 px-3 rounded-lg font-black text-[9px] uppercase tracking-widest transition-all shadow-sm active:scale-95 flex items-center gap-1.5 {{ $esVencido ? 'bg-rose-600 text-white hover:bg-rose-700' : 'bg-slate-800 dark:bg-slate-700 text-white hover:bg-black dark:hover:bg-slate-600' }}">
                                        ...
                                        <div class="h-4 w-[1px] bg-slate-100 dark:bg-slate-800 mx-0.5"></div>
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-10 text-center">
                                    <div class="flex flex-col items-center gap-2">
                                        <svg class="w-10 h-10 text-slate-200 dark:text-slate-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                                        <p class="text-slate-400 dark:text-slate-500 font-bold text-xs uppercase tracking-widest leading-loose">No hay operaciones registradas.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all border border-slate-100 dark:border-slate-800">
            ...
            <div class="p-8">
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        <h3 class="text-xl font-black text-[#00324D] dark:text-white leading-tight">Reingreso de Equipo</h3>
                ...
                <form id="formFinalizar" method="POST">
                    ...
                    <div class="mb-6 space-y-2">
                        ...
                        <textarea name="observaciones" rows="4" 
                                class="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl text-[13px] font-medium text-[#00324D] dark:text-white focus:ring-4 focus:ring-[#39A900]/5 focus:border-[#39A900] transition-all outline-none placeholder:text-slate-300 dark:placeholder:text-slate-600 resize-none leading-relaxed" 
                                placeholder="Describe el estado en el que se recibe el equipo..."></textarea>
                    </div>
                    <div class="flex flex-col gap-2">
                        <button type="submit" class="w-full py-4 bg-[#00324D] dark:bg-slate-800 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-[#002538] dark:hover:bg-slate-700 shadow-lg shadow-[#00324D]/20 transition-all hover:scale-[1.02] active:scale-95 border border-transparent dark:border-slate-700">
                            Confirmar Devolución
                        </button>
                        <button type="button" onclick="cerrarModal()" class="w-full py-3 text-slate-400 dark:text-slate-500 font-black text-[10px] uppercase tracking-widest hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                            Cancelar
                        </button>
                    </div>
                </form>
