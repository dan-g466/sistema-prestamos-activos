    ...
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-600 text-[10px] uppercase font-black tracking-widest mt-1">
                Auditoría de Préstamos <span class="mx-2 text-slate-300">|</span> Folio #{{ $prestamo->id }}
            </p>
        </div>
            ...
            <form action="{{ route('admin.prestamos.update', $prestamo) }}" method="POST" class="p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5 opacity-60">
                        <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Aprendiz Solicitante</label>
                        <div class="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl">
                            ...
                            <span class="text-sm font-bold text-slate-800">{{ $prestamo->user->name }}</span>
                        </div>
                    </div>

                    <div class="space-y-1.5 opacity-60">
                        <label class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Elemento Técnico</label>
                        <div class="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl">
                            <div class="w-8 h-8 rounded-xl bg-white flex items-center justify-center text-[#39A900] shadow-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"/></svg>
                            </div>
                            <span class="text-sm font-bold text-slate-800">{{ $prestamo->elemento->nombre }}</span>
                        </div>
                    </div>

                    {{-- Estado --}}
                    <div class="space-y-1.5">
                        <label for="estado" class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Estado de la Operación</label>
                        <div class="relative group/input">
                            ...
                            <select name="estado" id="estado" 
                                    class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:ring-4 focus:ring-[#39A900]/5 focus:border-[#39A900] transition-all outline-none appearance-none cursor-pointer">
                    ...
                    {{-- Fecha Límite --}}
                    <div class="space-y-1.5">
                        <label for="fecha_devolucion_esperada" class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Límite de Devolución</label>
                        <div class="relative group/input">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within/input:text-[#39A900] transition-colors">
                            ...
                            <input type="datetime-local" id="fecha_devolucion_esperada" name="fecha_devolucion_esperada" 
                                   ...
                                   class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:ring-4 focus:ring-[#39A900]/5 focus:border-[#39A900] transition-all outline-none">
                        </div>
                    </div>

                    {{-- Fecha Devolución Real --}}
                    <div class="space-y-1.5">
                        <label for="fecha_devolucion_real" class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Fecha de Devolución Real</label>
                        <div class="relative group/input">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within/input:text-[#39A900] transition-colors">
                ...
                <div class="mt-6 space-y-1.5">
                    <label for="observaciones" class="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 ml-1">Observaciones / Notas de Auditoría</label>
                    <textarea id="observaciones" name="observaciones" rows="4" 
                              placeholder="Escribe detalles sobre la entrega, estado del equipo o motivos de rechazo..."
                              class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:ring-4 focus:ring-[#39A900]/5 focus:border-[#39A900] transition-all outline-none placeholder:text-slate-300 resize-none leading-relaxed">{{ $prestamo->observaciones }}</textarea>
                </div>

                <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
                    <a href="{{ route('admin.prestamos.index') }}" 
                       class="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-colors">
                        Descartar
                    </a>
                    <button type="submit" 
                            class="px-8 py-3 bg-[#39A900] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-green-700 transition-all shadow-lg shadow-green-100 active:scale-95 flex items-center gap-2">
