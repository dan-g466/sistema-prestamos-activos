<x-admin-layout>
    <div class="mb-2 flex items-center justify-between">
        <div>
            ...
            <p class="text-black text-[10px] font-black uppercase tracking-widest mt-0.5">
                {{ $elemento->nombre }} <span class="mx-2 text-slate-300">|</span> 
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-[2rem] shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden relative group">
            <div class="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#39A900] to-[#00324D]"></div>
            
            <form action="{{ route('admin.elementos.update', $elemento) }}" method="POST" enctype="multipart/form-data" class="p-5 md:p-6 text-black">
                @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative">
                            <input type="text" id="nombre" name="nombre" value="{{ $elemento->nombre }}" required
                                   class="w-full pl-4 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold text-black focus:ring-4 focus:ring-[#39A900]/5 focus:border-[#39A900] transition-all outline-none">
                        </div>
                    </div>
                    ...
                    {{-- Estado --}}
                    <div class="space-y-1">
                        <label for="estado" class="block text-[9px] font-black uppercase tracking-[0.2em] text-black ml-1">Estado de Disponibilidad</label>
                    ...
                    <div class="md:col-span-2 space-y-1">
                        ...
                        <div class="flex items-center gap-4">
                            <div id="image-preview" class="w-20 h-20 bg-slate-50 rounded-2xl border-2 {{ $elemento->imagen ? 'border-solid border-[#39A900]/20' : 'border-dashed border-slate-200' }} flex items-center justify-center overflow-hidden">
                                @if($elemento->imagen)
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-[9px] text-slate-400 mt-1 font-bold">PNG, JPG o WEBP (Máx. 2MB). Dejar vacío para conservar la actual.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 space-y-1">
                    ...
                    <textarea id="descripcion" name="descripcion" rows="3"
                              class="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-medium text-black focus:ring-4 focus:ring-[#39A900]/5 focus:border-[#39A900] transition-all outline-none resize-none">{{ old('descripcion', $elemento->descripcion) }}</textarea>
                </div>
